...
/*virtual*/ void ReduceElementsNode<ElemType>::CopyTo(ComputationNodeBasePtr nodeP, const std::wstring& newName, const CopyNodeFlags flags) const /*override*/
{
    ...
    if (flags & CopyNodeFlags::copyNodeValue)
    {
        ...
        node->m_keepDimension = m_keepDimension;
    }
}
...
/*virtual*/ void ReduceElementsNode<ElemType>::Load(File& fstream, size_t modelVersion) /*override*/
{
    ...
    if (modelVersion >= CNTK_MODEL_VERSION_24)
        fstream >> m_keepDimension;
    else
        m_keepDimension = DefaultKeepDimensionSetting(m_axis);
...
/*virtual*/ void ReduceElementsNode<ElemType>::ForwardProp(const FrameRange& fr) /*override*/
{
    ...
    if (ReduceSequenceAxis())
    {
        ...
        input = ComputationNode<ElemType>::Unpack(GetSampleLayout(), InputRef(0).Value(), InputRef(0).GetMBLayout(), m_tempUnpackedData, m_tempScatterIndices, /*batchMajor=*/ true, &gapPadValue);
    }
    else
...
/*virtual*/ void ReduceElementsNode<ElemType>::Validate(bool isFinalValidationPass) /*override*/
{
    ...
    m_scale = (ElemType)1;
    if (ReduceAllAxes())
        Base::ValidateUnaryReduce(isFinalValidationPass, m_keepDimension);
    ...
    else
    {
        ...
        if (ReduceAllStaticAxes() || ReduceAllAxes())
        {
            ...
            dims = m_keepDimension ? SmallVector<size_t>(shape.GetRank(), 1) : SmallVector<size_t>({ 1 }); // entire sample is reduced to a scalar
        }
